...
console.error(aa.length);
console.error(Object.keys(bb).length);
console.assert(aa.length === Object.keys(bb).length);
cc = {};
  ...
  'section#content',
    ...
    Array.from(document.querySelectorAll(query)).forEach(function (element) {
        Array.from(element.childNodes).forEach(function (element) {
            if (!(element.classList && element.classList.contains('content-wrapper'))) {
                element.remove();
            }
        });
    });
